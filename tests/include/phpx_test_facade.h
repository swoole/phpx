...
namespace test_facade_detail {
inline Variant call(const char *name, const ArgList &args = {}) {
    return php::call(getFunction(name), args);
}
}  // namespace test_facade_detail

inline Variant array_fill(const Variant &start_index, const Variant &count, const Variant &value) {
    return test_facade_detail::call("array_fill", {start_index, count, value});
}

template <typename... Values>
inline Variant array_push(const Reference &array, const Values &...values) {
    return test_facade_detail::call("array_push", {&array, values...});
}

inline Variant array_flip(const Variant &array) {
    return test_facade_detail::call("array_flip", {array});
}

inline Variant class_exists(const Variant &class_name, const Variant &autoload = true) {
    return test_facade_detail::call("class_exists", {class_name, autoload});
}

inline Variant define(const Variant &name, const Variant &value, const Variant &case_insensitive = false) {
    return test_facade_detail::call("define", {name, value, case_insensitive});
}
...
inline Variant file_put_contents(const Variant &filename,
    ...
    return test_facade_detail::call("file_put_contents", {filename, data, flags, context});
}

inline Variant fmod(const Variant &num1, const Variant &num2) {
    return test_facade_detail::call("fmod", {num1, num2});
}
...
inline Variant fopen(const Variant &filename,
                     ...
                     const Variant &context = nullptr) {
    return test_facade_detail::call("fopen", {filename, mode, use_include_path, context});
}
...
inline Variant hash(const Variant &algorithm,
    ...
    return test_facade_detail::call("hash", {algorithm, data, binary, options});
}
...
inline Variant http_build_query(const Variant &data,
    ...
    return test_facade_detail::call("http_build_query", {data, numeric_prefix, arg_separator, encoding_type});
}

inline Variant in_array(const Variant &needle, const Variant &haystack, const Variant &strict = false) {
    return test_facade_detail::call("in_array", {needle, haystack, strict});
}

inline Variant ini_get(const Variant &option) {
    return test_facade_detail::call("ini_get", {option});
}

inline Variant is_dir(const Variant &filename) {
    return test_facade_detail::call("is_dir", {filename});
}
