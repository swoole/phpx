...
TEST(is_set, array_dim_fetch_basic) {
    // 测试基本数组访问 - 模拟 PHP isset() 行为
    Array arr;
...
TEST(is_set, property_fetch_basic) {
    // 测试对象属性访问
...
TEST(is_set, property_fetch_nested) {
    // 测试嵌套对象属性访问
...
TEST(exists, edge_cases) {
    ...
    // 空变体
    Variant empty_v;
    ASSERT_FALSE(exists(empty_v, {{ArrayDimFetch, "key"}}));
    ASSERT_FALSE(exists(empty_v, {{PropertyFetch, "prop"}}));

    // null 值
    Variant null_v = nullptr;
    ASSERT_FALSE(exists(null_v, {{ArrayDimFetch, "key"}}));
    ASSERT_FALSE(exists(null_v, {{PropertyFetch, "prop"}}));

    // 字符串值（不支持数组或属性访问）
    Variant str_v = "string";
    ASSERT_FALSE(exists(str_v, {{ArrayDimFetch, "key"}}));
    ASSERT_FALSE(exists(str_v, {{PropertyFetch, "prop"}}));

    // 整数值（不支持数组或属性访问）
    Variant int_v = 123;
    ASSERT_FALSE(exists(int_v, {{ArrayDimFetch, "key"}}));
    ASSERT_FALSE(exists(int_v, {{PropertyFetch, "prop"}}));
}

TEST(is_set, complex_chains) {
    // 测试复杂的访问链
...
TEST(is_set, string_numeric_keys) {
    // 测试字符串形式的数字键
    Array mixed_arr;
    ...
    // 字符串数字键（存在，包括空值）
    ASSERT_TRUE(exists(v, {{ArrayDimFetch, "0"}}));
    ASSERT_TRUE(is_set(v, {{ArrayDimFetch, "1"}}));  // 空字符串

    // null值应该返回false
    ASSERT_FALSE(is_set(v, {{ArrayDimFetch, "2"}}));

    // 实际数字键（应该找不到）
    ASSERT_FALSE(is_set(v, {{ArrayDimFetch, 0}}));
}

TEST(exists, comparison_with_empty) {
    // 测试 is_set 和 empty 的区别
