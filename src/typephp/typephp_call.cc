...
php::Variant php::FunctionCallCacheSlot::call(const Variant &func, Args &args, zend_array *named_args) {
    if (UNEXPECTED(!func.isString())) {
        return php::call(func, args, named_args);
    }
    ...
    if (polymorphic_cache_ != nullptr) {
        ...
        if (EXPECTED(cached != nullptr)) {
            return invokeCached(func, nullptr, cached, args, named_args);
        }
    } else if (EXPECTED(name_ != nullptr && zend_string_equals(name_, name))) {
        return invokeCached(func, nullptr, &cache_, args, named_args);
    }
    ...
    return invokeCached(func, resolved.object, &resolved, args, named_args);
}

php::Variant php::MethodCallCacheSlot::call(const Variant &object,
    ...
    if (UNEXPECTED(!method.isString()) || UNEXPECTED(polymorphic_)) {
        ...
        return invokeCached(method, zend_object, &resolved, args, named_args);
    }
