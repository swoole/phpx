...
Variant set_error_handler(const Variant &callback, const Variant &error_levels) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[93]);
    }
...
Variant set_exception_handler(const Variant &callback) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[95]);
    }
...
Variant restore_exception_handler() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[96]);
    }
    return call(fn, {});
}
Variant get_declared_classes() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[97]);
    }
    return call(fn, {});
}
Variant get_declared_traits() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[98]);
    }
    return call(fn, {});
}
Variant get_declared_interfaces() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[99]);
    }
    return call(fn, {});
}
Variant get_defined_functions(const Variant &exclude_disabled) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[100]);
    }
...
Variant get_defined_vars() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[101]);
    }
    return call(fn, {});
}
Variant get_resource_type(const Variant &resource) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[102]);
    }
...
Variant get_resource_id(const Variant &resource) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[103]);
    }
    return call(fn, {resource});
}
Variant get_resources(const Variant &type) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[104]);
    }
    return call(fn, {type});
}
Variant get_loaded_extensions(const Variant &zend_extensions) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[105]);
    }
...
Variant get_defined_constants(const Variant &categorize) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[106]);
    }
...
Variant debug_backtrace(const Variant &options, const Variant &limit) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[107]);
    }
...
Variant debug_print_backtrace(const Variant &options, const Variant &limit) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[108]);
    }
    return call(fn, {options, limit});
}
Variant extension_loaded(const Variant &extension) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[109]);
    }
...
Variant get_extension_funcs(const Variant &extension) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[110]);
    }
    return call(fn, {extension});
}
Variant zend_thread_id() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[111]);
    }
    return call(fn, {});
}
Variant gc_mem_caches() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[112]);
    }
    return call(fn, {});
}
Variant gc_collect_cycles() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[113]);
    }
    return call(fn, {});
}
Variant gc_enabled() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[114]);
    }
    return call(fn, {});
}
Variant gc_enable() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[115]);
    }
    return call(fn, {});
}
Variant gc_disable() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[116]);
    }
    return call(fn, {});
}
Variant gc_status() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[117]);
    }
    return call(fn, {});
}
}  // namespace php
