...
Variant curl_close(const CurlHandle &handle) {
    ...
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[202]);
    }
...
CurlHandle curl_copy_handle(const CurlHandle &handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[203]);
    }
...
Variant curl_errno(const CurlHandle &handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[204]);
    }
    return call(_fn, {handle.getObject()});
}
...
Variant curl_error(const CurlHandle &handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[205]);
    }
    return call(_fn, {handle.getObject()});
}
...
Variant curl_escape(const CurlHandle &handle, const Variant &string) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[206]);
    }
...
Variant curl_unescape(const CurlHandle &handle, const Variant &string) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[207]);
    }
    return call(_fn, {handle.getObject(), string});
}
...
Variant curl_multi_setopt(const CurlMultiHandle &multi_handle, const Variant &option, const Variant &value) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[208]);
    }
...
Variant curl_exec(const CurlHandle &handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[209]);
    }
    return call(_fn, {handle.getObject()});
}
...
Variant curl_file_create(const Variant &filename, const Variant &mime_type, const Variant &posted_filename) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[210]);
    }
    return call(_fn, {filename, mime_type, posted_filename});
}
Variant curl_getinfo(const CurlHandle &handle, const Variant &option) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[211]);
    }
...
CurlHandle curl_init(const Variant &url) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[212]);
    }
...
Variant curl_upkeep(const CurlHandle &handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[213]);
    }
    return call(_fn, {handle.getObject()});
}
...
Variant curl_multi_add_handle(const CurlMultiHandle &multi_handle, const CurlHandle &handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[214]);
    }
...
Variant curl_multi_close(const CurlMultiHandle &multi_handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[215]);
    }
...
Variant curl_multi_errno(const CurlMultiHandle &multi_handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[216]);
    }
    return call(_fn, {multi_handle.getObject()});
}
...
Variant curl_multi_exec(const CurlMultiHandle &multi_handle, const Reference &still_running) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[217]);
    }
...
Variant curl_multi_getcontent(const CurlHandle &handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[218]);
    }
    return call(_fn, {handle.getObject()});
}
...
Variant curl_multi_info_read(const CurlMultiHandle &multi_handle, const Reference &queued_messages) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[219]);
    }
...
CurlMultiHandle curl_multi_init() {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[220]);
    }
...
Variant curl_multi_remove_handle(const CurlMultiHandle &multi_handle, const CurlHandle &handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[221]);
    }
    return call(_fn, {multi_handle.getObject(), handle.getObject()});
}
...
Variant curl_multi_select(const CurlMultiHandle &multi_handle, const Variant &timeout) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[222]);
    }
...
Variant curl_multi_strerror(const Variant &error_code) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[223]);
    }
...
Variant curl_pause(const CurlHandle &handle, const Variant &flags) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[224]);
    }
...
Variant curl_reset(const CurlHandle &handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[225]);
    }
    return call(_fn, {handle.getObject()});
}
...
Variant curl_setopt_array(const CurlHandle &handle, const Variant &options) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[226]);
    }
...
Variant curl_setopt(const CurlHandle &handle, const Variant &option, const Variant &value) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[227]);
    }
...
Variant curl_share_close(const CurlShareHandle &share_handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[228]);
    }
    return call(_fn, {share_handle.getObject()});
}
Variant curl_share_close(const Variant &share_handle) {
    ...
    return call(_fn, {share_handle});
}
Variant curl_share_errno(const CurlShareHandle &share_handle) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[229]);
    }
    return call(_fn, {share_handle.getObject()});
}
...
CurlShareHandle curl_share_init() {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[230]);
    }
    return CurlShareHandle(Object(call(_fn, {})));
}
Variant curl_share_setopt(const CurlShareHandle &share_handle, const Variant &option, const Variant &value) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[231]);
    }
    return call(_fn, {share_handle.getObject(), option, value});
}
Variant curl_share_setopt(const Variant &share_handle, const Variant &option, const Variant &value) {
    ...
    return call(_fn, {share_handle, option, value});
}
Variant curl_share_strerror(const Variant &error_code) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[232]);
    }
    return call(_fn, {error_code});
}
Variant curl_strerror(const Variant &error_code) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[233]);
    }
    return call(_fn, {error_code});
}
Variant curl_version() {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[234]);
    }
    return call(_fn, {});
}
}  // namespace php
