...
XMLWriter xmlwriter_open_uri(const Variant &uri) {
    ...
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3358]);
    }
    auto _rv = call(_fn, {uri});
    if (!_rv.toBool()) {
...
XMLWriter xmlwriter_open_memory() {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3359]);
    }
    auto _rv = call(_fn, {});
    if (!_rv.toBool()) {
...
Variant xmlwriter_set_indent(const XMLWriter &writer, const Variant &enable) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3360]);
    }
    return call(_fn, {writer.getObject(), enable});
}
Variant xmlwriter_set_indent(const Variant &writer, const Variant &enable) {
    ...
    return call(_fn, {writer, enable});
}
Variant xmlwriter_set_indent_string(const XMLWriter &writer, const Variant &indentation) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3361]);
    }
    return call(_fn, {writer.getObject(), indentation});
}
Variant xmlwriter_set_indent_string(const Variant &writer, const Variant &indentation) {
    ...
    return call(_fn, {writer, indentation});
}
Variant xmlwriter_start_comment(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3362]);
    }
    return call(_fn, {writer.getObject()});
}
Variant xmlwriter_start_comment(const Variant &writer) {
    ...
    return call(_fn, {writer});
}
Variant xmlwriter_end_comment(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3363]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_start_attribute(const XMLWriter &writer, const Variant &name) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3364]);
    }
    return call(_fn, {writer.getObject(), name});
}
Variant xmlwriter_start_attribute(const Variant &writer, const Variant &name) {
    ...
    return call(_fn, {writer, name});
}
Variant xmlwriter_end_attribute(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3365]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_write_attribute(const XMLWriter &writer, const Variant &name, const Variant &value) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3366]);
    }
    return call(_fn, {writer.getObject(), name, value});
}
Variant xmlwriter_write_attribute(const Variant &writer, const Variant &name, const Variant &value) {
    ...
    return call(_fn, {writer, name, value});
}
Variant xmlwriter_start_attribute_ns(const XMLWriter &writer,
    ...
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3367]);
    }
    return call(_fn, {writer.getObject(), prefix, name, namespace_});
}
Variant xmlwriter_start_attribute_ns(const Variant &writer,
    ...
    return call(_fn, {writer, prefix, name, namespace_});
}
Variant xmlwriter_write_attribute_ns(const XMLWriter &writer,
    ...
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3368]);
    }
    return call(_fn, {writer.getObject(), prefix, name, namespace_, value});
}
Variant xmlwriter_write_attribute_ns(const Variant &writer,
    ...
    return call(_fn, {writer, prefix, name, namespace_, value});
}
Variant xmlwriter_start_element(const XMLWriter &writer, const Variant &name) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3369]);
    }
    return call(_fn, {writer.getObject(), name});
}
...
Variant xmlwriter_end_element(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3370]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_full_end_element(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3371]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_start_element_ns(const XMLWriter &writer,
    ...
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3372]);
    }
    return call(_fn, {writer.getObject(), prefix, name, namespace_});
}
...
Variant xmlwriter_write_element(const XMLWriter &writer, const Variant &name, const Variant &content) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3373]);
    }
    return call(_fn, {writer.getObject(), name, content});
}
Variant xmlwriter_write_element(const Variant &writer, const Variant &name, const Variant &content) {
    ...
    return call(_fn, {writer, name, content});
}
Variant xmlwriter_write_element_ns(const XMLWriter &writer,
    ...
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3374]);
    }
    return call(_fn, {writer.getObject(), prefix, name, namespace_, content});
}
Variant xmlwriter_write_element_ns(const Variant &writer,
    ...
    return call(_fn, {writer, prefix, name, namespace_, content});
}
Variant xmlwriter_start_pi(const XMLWriter &writer, const Variant &target) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3375]);
    }
    return call(_fn, {writer.getObject(), target});
}
Variant xmlwriter_start_pi(const Variant &writer, const Variant &target) {
    ...
    return call(_fn, {writer, target});
}
Variant xmlwriter_end_pi(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3376]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_write_pi(const XMLWriter &writer, const Variant &target, const Variant &content) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3377]);
    }
    return call(_fn, {writer.getObject(), target, content});
}
Variant xmlwriter_write_pi(const Variant &writer, const Variant &target, const Variant &content) {
    ...
    return call(_fn, {writer, target, content});
}
Variant xmlwriter_start_cdata(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3378]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_end_cdata(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3379]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_write_cdata(const XMLWriter &writer, const Variant &content) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3380]);
    }
    return call(_fn, {writer.getObject(), content});
}
...
Variant xmlwriter_write_raw(const XMLWriter &writer, const Variant &content) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3382]);
    }
    return call(_fn, {writer.getObject(), content});
}
...
Variant xmlwriter_start_document(const XMLWriter &writer,
    ...
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3383]);
    }
    return call(_fn, {writer.getObject(), version, encoding, standalone});
}
Variant xmlwriter_start_document(const Variant &writer,
    ...
    return call(_fn, {writer, version, encoding, standalone});
}
Variant xmlwriter_end_document(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3384]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_write_comment(const XMLWriter &writer, const Variant &content) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3385]);
    }
    return call(_fn, {writer.getObject(), content});
}
...
Variant xmlwriter_start_dtd(const XMLWriter &writer,
    ...
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3386]);
    }
    return call(_fn, {writer.getObject(), qualified_name, public_id, system_id});
}
Variant xmlwriter_start_dtd(const Variant &writer,
    ...
    return call(_fn, {writer, qualified_name, public_id, system_id});
}
Variant xmlwriter_end_dtd(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3387]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_write_dtd(const XMLWriter &writer,
    ...
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3388]);
    }
    return call(_fn, {writer.getObject(), name, public_id, system_id, content});
}
Variant xmlwriter_write_dtd(const Variant &writer,
    ...
    return call(_fn, {writer, name, public_id, system_id, content});
}
Variant xmlwriter_start_dtd_element(const XMLWriter &writer, const Variant &qualified_name) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3389]);
    }
    return call(_fn, {writer.getObject(), qualified_name});
}
Variant xmlwriter_start_dtd_element(const Variant &writer, const Variant &qualified_name) {
    ...
    return call(_fn, {writer, qualified_name});
}
Variant xmlwriter_end_dtd_element(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3390]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_write_dtd_element(const XMLWriter &writer, const Variant &name, const Variant &content) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3391]);
    }
    return call(_fn, {writer.getObject(), name, content});
}
...
Variant xmlwriter_start_dtd_attlist(const XMLWriter &writer, const Variant &name) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3392]);
    }
    return call(_fn, {writer.getObject(), name});
}
...
Variant xmlwriter_end_dtd_attlist(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3393]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_write_dtd_attlist(const XMLWriter &writer, const Variant &name, const Variant &content) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3394]);
    }
    return call(_fn, {writer.getObject(), name, content});
}
...
Variant xmlwriter_start_dtd_entity(const XMLWriter &writer, const Variant &name, const Variant &is_param) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3395]);
    }
    return call(_fn, {writer.getObject(), name, is_param});
}
Variant xmlwriter_start_dtd_entity(const Variant &writer, const Variant &name, const Variant &is_param) {
    ...
    return call(_fn, {writer, name, is_param});
}
Variant xmlwriter_end_dtd_entity(const XMLWriter &writer) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3396]);
    }
    return call(_fn, {writer.getObject()});
}
...
Variant xmlwriter_write_dtd_entity(const XMLWriter &writer,
    ...
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3397]);
    }
    return call(_fn, {writer.getObject(), name, content, is_param, public_id, system_id, notation_data});
}
Variant xmlwriter_write_dtd_entity(const Variant &writer,
    ...
    return call(_fn, {writer, name, content, is_param, public_id, system_id, notation_data});
}
Variant xmlwriter_output_memory(const XMLWriter &writer, const Variant &flush) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3398]);
    }
    return call(_fn, {writer.getObject(), flush});
}
Variant xmlwriter_output_memory(const Variant &writer, const Variant &flush) {
    ...
    return call(_fn, {writer, flush});
}
Variant xmlwriter_flush(const XMLWriter &writer, const Variant &empty) {
    static THREAD_LOCAL zend_function *_fn = nullptr;
    if (UNEXPECTED(!_fn)) {
        _fn = getFunction(LITERAL_STRING[3399]);
    }
    return call(_fn, {writer.getObject(), empty});
}
