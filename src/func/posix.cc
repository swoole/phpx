...
Variant posix_kill(const Variant &process_id, const Variant &signal) {
    ...
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[1157]);
    }
...
Variant posix_getpid() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[1158]);
    }
    return call(fn, {});
}
Variant posix_getppid() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[1159]);
    }
    return call(fn, {});
}
Variant posix_getuid() {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[1160]);
    }
    return call(fn, {});
}
Variant posix_setuid(const Variant &user_id) {
    static THREAD_LOCAL zend_function *fn = nullptr;
    if (UNEXPECTED(!fn)) {
        fn = getFunction(LITERAL_STRING[1161]);
    }
