...
TYPEPHP_EMBED_PRE_SHUTDOWN_FUNCTION(TYPEPHP_PROJECT_NAME);

TYPEPHP_RUNTIME_INIT_FUNCTION(TYPEPHP_PROJECT_NAME) {
    return typephp_runtime_start(
        TYPEPHP_EMBED_GET_MODULE(TYPEPHP_PROJECT_NAME),
        TYPEPHP_EMBED_PRE_SHUTDOWN(TYPEPHP_PROJECT_NAME),
        argc,
        argv);
}
