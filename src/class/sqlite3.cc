...
SQLite3Exception::SQLite3Exception(const Variant &message, const Variant &code, const Variant &previous) {
    this_ = newObject(LITERAL_STRING[2048], {message, code, previous});
}
...
SQLite3::SQLite3(const Variant &filename, const Variant &flags, const Variant &encryption_key) {
    this_ = newObject(LITERAL_STRING[2049], {filename, flags, encryption_key});
}
...
Variant SQLite3::version() {
    static THREAD_LOCAL zend_function *_method_fn = nullptr;
    if (UNEXPECTED(!_method_fn)) {
        _method_fn = php::getMethod(LITERAL_STRING[2049], LITERAL_STRING[2051]);
    }
...
Variant SQLite3::lastErrorMsg() {
    static THREAD_LOCAL zend_function *_method_fn = nullptr;
    if (UNEXPECTED(!_method_fn)) {
        _method_fn = php::getMethod(this_.ce(), LITERAL_STRING[2055]);
    }
    return this_.call(_method_fn, {});
}
Variant SQLite3::changes() {
    static THREAD_LOCAL zend_function *_method_fn = nullptr;
    if (UNEXPECTED(!_method_fn)) {
        _method_fn = php::getMethod(this_.ce(), LITERAL_STRING[2056]);
    }
    return this_.call(_method_fn, {});
}
...
Variant SQLite3::escapeString(const Variant &string) {
    static THREAD_LOCAL zend_function *_method_fn = nullptr;
    if (UNEXPECTED(!_method_fn)) {
        _method_fn = php::getMethod(LITERAL_STRING[2049], LITERAL_STRING[2060]);
    }
