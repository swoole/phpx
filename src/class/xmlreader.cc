...
Variant XMLReader::fromUri(const Variant &uri, const Variant &encoding, const Variant &flags) {
    static Variant fn { ZEND_STRL("XMLReader::fromUri"), true };
    return fn({uri, encoding, flags});
}

Variant XMLReader::fromStream(const Variant &stream, const Variant &encoding, const Variant &flags, const Variant &document_uri) {
    static Variant fn { ZEND_STRL("XMLReader::fromStream"), true };
    return fn({stream, encoding, flags, document_uri});
}
...
Variant XMLReader::fromString(const Variant &source, const Variant &encoding, const Variant &flags) {
    static Variant fn { ZEND_STRL("XMLReader::fromString"), true };
    return fn({source, encoding, flags});
}
