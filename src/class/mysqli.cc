...
mysqli_sql_exception::mysqli_sql_exception(const Variant &message, const Variant &code, const Variant &previous) {
    this_ = newObject(LITERAL_STRING[854], {message, code, previous});
}
mysqli_driver::mysqli_driver() {
    this_ = newObject(LITERAL_STRING[855]);
}
...
mysqli_result::mysqli_result(const mysqli &mysql, const Variant &result_mode) {
    this_ = newObject(LITERAL_STRING[901], {mysql.getObject(), result_mode});
}
...
mysqli::mysqli(const Variant &hostname,
    ...
    this_ = newObject(LITERAL_STRING[856], {hostname, username, password, database, port, socket});
}
...
Variant mysqli::poll(const Reference &read,
    ...
    static THREAD_LOCAL zend_function *_method_fn = nullptr;
    if (UNEXPECTED(!_method_fn)) {
        _method_fn = php::getMethod(LITERAL_STRING[856], LITERAL_STRING[879]);
    }
...
mysqli_stmt::mysqli_stmt(const mysqli &mysql, const Variant &query) {
    this_ = newObject(LITERAL_STRING[915], {mysql.getObject(), query});
}
