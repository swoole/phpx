...
Variant XMLWriter::writeDtdEntity(const Variant &name,
    ...
    return this_.exec(LITERAL_STRING[2491], {name, content, is_param, public_id, system_id, notation_data});
}
Variant XMLWriter::outputMemory(const Variant &flush) {
    return this_.exec(LITERAL_STRING[2492], {flush});
}
Variant XMLWriter::flush(const Variant &empty) {
    return this_.exec(LITERAL_STRING[1950], {empty});
}
}  // namespace php
