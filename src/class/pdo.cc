...
PDOException::PDOException(const Variant &message, const Variant &code, const Variant &previous) {
    this_ = newObject(LITERAL_STRING[1038], {message, code, previous});
}
PDOStatement::PDOStatement() {
    this_ = newObject(LITERAL_STRING[1050]);
}
...
Variant PDOStatement::fetchObject(const Variant &class_, const Variant &constructor_args) {
    static THREAD_LOCAL zend_function *_method_fn = nullptr;
    if (UNEXPECTED(!_method_fn)) {
        _method_fn = php::getMethod(this_.ce(), LITERAL_STRING[1059]);
    }
...
Variant PDOStatement::nextRowset() {
    static THREAD_LOCAL zend_function *_method_fn = nullptr;
    if (UNEXPECTED(!_method_fn)) {
        _method_fn = php::getMethod(this_.ce(), LITERAL_STRING[1061]);
    }
    return this_.call(_method_fn, {});
}
...
PDO::PDO(const Variant &dsn, const Variant &username, const Variant &password, const Variant &options) {
    this_ = newObject(LITERAL_STRING[1039], {dsn, username, password, options});
}
Variant PDO::connect(const Variant &dsn, const Variant &username, const Variant &password, const Variant &options) {
    static THREAD_LOCAL zend_function *_method_fn = nullptr;
    if (UNEXPECTED(!_method_fn)) {
        _method_fn = php::getMethod(LITERAL_STRING[1039], LITERAL_STRING[863]);
    }
...
Variant PDO::beginTransaction() {
    static THREAD_LOCAL zend_function *_method_fn = nullptr;
    if (UNEXPECTED(!_method_fn)) {
        _method_fn = php::getMethod(this_.ce(), LITERAL_STRING[1041]);
    }
    return this_.call(_method_fn, {});
}
Variant PDO::commit() {
...
Variant PDO::getAvailableDrivers() {
    static THREAD_LOCAL zend_function *_method_fn = nullptr;
    if (UNEXPECTED(!_method_fn)) {
        _method_fn = php::getMethod(LITERAL_STRING[1039], LITERAL_STRING[1046]);
    }
...
PDORow::PDORow() {
    this_ = newObject(LITERAL_STRING[1064]);
}
}  // namespace php
